...
def install_router_hooks(model: OlmoeForCausalLM) -> dict:
    ...
    def make_hook(layer_idx: int):
        def hook(module, input_, output):
            # output is (routing_weights, selected_experts) from OlmoeRouter
            # selected_experts: (batch*seq_len, top_k) LongTensor of expert indices
            selected_experts = output[1].detach().cpu().numpy().astype(np.int16)
...
def main():
    ...
    parser.add_argument(
        ...
        help="Load the model in 8-bit via bitsandbytes. Roughly halves memory "
             "again vs bf16 (~7GB weights vs ~14GB). Requires `pip install "
             "bitsandbytes accelerate`. NOTE: this measures routing on a "
             "quantized model, not full precision -- see the note printed "
             "at the top of the report. Router decisions are a discrete "
             "argmax over the router's output, so quantization of the main "
             "model is a second-order effect on which experts get picked, "
             "but it is not zero. Treat results as directional evidence, "
    ...
    if args.load_in_8bit:
        # 8-bit requires device_map="auto" regardless of device, so
        # bitsandbytes can place/manage the quantized layers itself.
